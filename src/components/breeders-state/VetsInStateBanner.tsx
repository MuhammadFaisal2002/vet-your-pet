...
interface VetsInStateBannerProps {
  stateName: string;
  stateSlug: string;
}

export default function VetsInStateBanner({ stateName, stateSlug }: VetsInStateBannerProps) {
  return (
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex items-center gap-4">
              <span className="w-12 h-12 rounded-full bg-white border border-pet-stroke flex items-center justify-center flex-shrink-0">
                <Stethoscope className="w-6 h-6 text-brand-red" aria-hidden="true" />
              </span>
              <div>
                ...
                <p className="text-lg md:text-xl font-bold text-brand-dark leading-snug">
                  Need a vet too? Browse verified veterinarians in {stateName}
                </p>
              </div>
            </div>
